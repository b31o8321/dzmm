...
**体验与能力提升：引导修复、多题材健壮性、战斗入口**

### 新增
- AI 创作页接入 genre 预设选择器（6 预设，选中即填入题材/基调/核心冲突）
- 启用 combat 的 Run 在游玩页显示战斗按钮（攻击→命令引擎 d20 结算→HP/倒下态反馈）
- SillyTavern v3 角色卡高频字段导入保留与原生卡导出回填
- D20 边境前哨预设世界（战斗 + 章节 + 结局）
- genre 骨架按预设参数化（四题材章节名/选择标签互不相同，自由文本回退默认）
- 开场叙事与 NPC 首句台词变体池（每骨架 4 条，按世界+主角轮换）
- 首启模型档案引导（世界页空列表 + AI 创作页置灰原因提示）
- DZMM_NARRATION_TIMEOUT / DZMM_MODEL_REQUEST_TIMEOUT 环境变量（本地大模型）

### 修复
- 协议切换不再覆盖已自定义的 Base URL
- 自由行动命中选项标签时给出引导提示（不再静默当作自由行动）
- 修复器确定性吞掉弱模型草案的六类形状漂移（不再整体拒绝）
- qwen3 泄漏清洗（括号指令/Start of Game/空 DZMM_ACTIONS 不再出现在正文）
- 战斗区空目标提示、回合表单空输入提示、导出按钮 title tooltip

### 内部
- cutover 完成：vnext 树提升为唯一产品树，旧版归档（ADR-010 全程）
- Director 长线节奏注入（ADR-012，后台异步、静默降级）
- SillyTavern v3 互通、portable bundle、模型档案完整 CRUD、回合 undo
