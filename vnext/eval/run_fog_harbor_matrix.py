...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="dzmm-vnext-fog-matrix-") as temporary:
        ...
        with TestClient(app, raise_server_exceptions=False) as client:
            ...
            for route_name, (choices, expected_ending) in ROUTES.items():
                ...
                payload.update(
                    {
                        "request_id": f"fog-matrix-compose-{route_name}",
                ...
                for revision, choice_id in enumerate(choices):
                    ...
                    response = client.post(
                        ...
                        json={
                            "request_id": f"fog-{route_name}-{revision + 1}",
                ...
                if state["revision"] != len(choices) or state["ending"]["id"] != expected_ending:
                    raise RuntimeError(f"{route_name} did not reach {expected_ending}")
                assert first_turn_id is not None
                rollback = client.post(
                    f"/api/v2/runs/{run_id}/rollbacks",
                    json={
                        "request_id": f"fog-{route_name}-rollback",
                ...
                if restored["chapter"]["id"] != "ch2" or restored["ending"] is not None:
                    raise RuntimeError(f"{route_name} rollback did not restore its second chapter")
    ...
    payload = {
        ...
        "total_choice_turns": sum(len(choices) for choices, _ending in ROUTES.values()),
        "latency_seconds": {
